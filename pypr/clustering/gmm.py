...
def gmm_init(X, K, verbose = False,
    ...
    if cluster_init == 'sample':
        ...
        raise ValueError("Unknown initialization of EM of MoG centers.")
...
##                            #c = c + np.diag(np.ones(c.shape[0]) * diag_add )
        ...
        if old_logL != np.NaN:
            if verbose:
                print("iteration=", i, " delta log likelihood=", old_logL - logL)
...
def cond_dist(Y, centroids, ccov, mc):
    ...
    set_idx = np.nonzero(1. - np.isnan(Y))[0]
...
def cond_moments(X, centroids, ccov, mc):
    ...
    set_idx = np.nonzero(1. - np.isnan(X))[0]
    new_idx = np.concatenate((not_set_idx, set_idx))
